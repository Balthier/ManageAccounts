...
if ("Y","Yes" -contains $Ad01Continue) {
	...
	## User Variables ##
	####################
	Do {$forename = Read-host "Forename (a-Z Only)"} until ($forename -Match '^(\w|-)*$')
	Do {$surname = Read-host "Surname (a-Z Only)"} until ($surname -Match '^(\w|-)*$')
	$password = "Autocab1" | ConvertTo-SecureString -AsPlainText -Force
	Do {$JobTitle = Read-Host "Job Title (a-Z, spaces, Only)"} until ($JobTitle -Match '^(\w| )*$')
	Do {$Dept = Read-Host "Department (a-Z, spaces, Only)"} until ($Dept -Match '^(\w| )*$')
	$f = $forename.Trim()
	$s = $surname.Trim()
	$jt = $JobTitle.Trim()
	$fn = $forename +" "+ $surname
	...
	$licenses = Get-MsolAccountSku | Where { $_.AccountSkuId -like "*O365_BUSINESS_PREMIUM*"}
